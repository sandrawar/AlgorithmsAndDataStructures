...
namespace ASD2
{
    ...
    public class Lab10TestModule : TestModule
    {
        ...
        public override void PrepareTestSets()
        {
            ...
            var mycielskiTestCase = BuildMycielskiTestCases();

            TestSets["SmallTestCases"] = smallTestCases;
            TestSets["LargeTestCases"] = largeTestCases;
            TestSets["Mycielski"] = mycielskiTestCase;
        }

        private TestSet BuildMycielskiTestCases()
        {
            var tests = new TestSet(new GraphColorer(), "Testy Mycielskiego");
            Graph genMyc(int i)
            {
                if (i == 2)
                {
                ...
                return Mycielskian(genMyc(i - 1));
            }


            var gen = Enumerable.Range(3, 4).Select(gen =>
            {
                var g = genMyc(gen);
                return new GraphColoringTestCase(g, gen, 9, $"Mycielski {g.VertexCount}");
            });
            foreach (var t in gen)
                tests.TestCases.Add(t);
        ...
        private TestSet BuildSmallTestCases()
        {
            ...
            {
                Graph G = new Graph(0);
                tests.TestCases.Add(new GraphColoringTestCase(G, 0, 1, "graf pusty"));
            }
